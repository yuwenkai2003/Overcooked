...
class Runner(object):
    ...
    def save(self, episode=0):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            save_dir = str(episode)
            os.makedirs(save_dir, exist_ok=True)
            #torch.save(self.policy_actor.state_dict(), os.path.join(save_dir, "actor.pt"))
            policy_actor = self.trainer.policy.actor
            # torch.save(policy_actor.state_dict(), str(episode) + "/actor.pt")
            torch.save(policy_actor.state_dict(), os.path.join(save_dir, "actor.pt"))
            policy_critic = self.trainer.policy.critic
            #torch.save(policy_critic.state_dict(), str(episode) + "/critic.pt")
            torch.save(policy_critic.state_dict(), os.path.join(save_dir, "critic.pt"))
    ...
    def restore(self, model_dir):
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            ...
            policy_actor_state_dict = torch.load(str(3060) + '/actor.pt')
            self.policy.actor.load_state_dict(policy_actor_state_dict)
            if not self.all_args.use_render:
                policy_critic_state_dict = torch.load(str(3060) + '/critic.pt')
