...
class MapSetting:
    ...
    max_num_timesteps: int = 200    # max number of timesteps
...
class OvercookedEnvironment(gym.Env):
    ...
    def reset(self):
        ...
        # return np.concatenate([np.random.rand(1) for _ in range(0, 1536)])
        ...
        for k in range(1):
            total_action_space = []
            total_action_space.append(spaces.Discrete(4))
            ...
            # obs_dim = len(self.get_obs(agent, self.generate_obs()[self.get_werewolf_index(k)]))
            # obs_dim = 3
            # share_obs_dim += obs_dim
            self.observation_space.append(spaces.Box(low=-np.inf, high=+np.inf,
                                                     shape=(24, 4, self.world.height), dtype=np.float32))
        self.share_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(24, 4, self.world.height), dtype=np.float32) for _ in range(1)]
    ...
    def get_current_state(self):
        ...
        result = [v for k, v in gridsquare_map.items()] + [v for k, v in obj_map.items()] + [v for k, v in
                                                                                       agent_map.items()]
        result0 = [v[0: 4] for k, v in agent_map.items() if k == 'agent-1'] + [v[0: 4] for k, v in obj_map.items()]
        self.result0 = result0
        #print(result0[0])
        result1 = [v[3:] for k, v in agent_map.items() if k == 'agent-2'] + [v[3:] for k, v in obj_map.items()]
        self.result1 = result1
        # print(result0)
        # print(len(result0))
        # print(obj_map['ChoppedTomato'])
        self.result = result
        # print(len(result))
        result = {
