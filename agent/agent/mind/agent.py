...
task_list = ['HighTask',
             ...
             'Prepare Alice Ingredients', 'Prepare Bob Ingredients', 'Prepare Cathy Ingredients', 'Prepare David Ingredients',
             'Cook Alice Soup', 'Cook Bob Soup', 'Cook Cathy Soup', 'Cook David Soup',
             'Plate Alice Soup', 'Plate Bob Soup', 'Plate Cathy Soup', 'Plate David Soup',
             'Serve Alice Soup', 'Serve Bob Soup', 'Serve Cathy Soup', 'Serve David Soup',
             'Drop', 'Clean_', 'Putout']


def getHighTaskID(task: str):
...
class HLAagent:
    ...
    def __call__(self, env: EnvState):
        ...
        while True:
            ...
            if self._task is None:
                continue
            #print(self._task.__str__())
            HighTaskID = getHighTaskID(self._task.__str__())
            #print(HighTaskID)
            env.cur_HighTask = HighTaskID
            # print(self._task)
            # print(env)
            #print(type(self._task))
...
class FMOAagent:
    ...
    def __call__(self, env: EnvState):
        ...
        while True:
            ...
            HighTaskID = getHighTaskID(self._task.__str__())
            #print(HighTaskID)
            print(env.agent_idx, self._task.__str__())
            env.cur_HighTask = HighTaskID
            state, move, msg = self._task(env)
            if state == HighTask.Working:  # working
                self._mov_hist[-1]['status'] = 'Ongoing. ' + msg
