    ...
    def __call__(self, env: EnvState):
        ...
        while not self._last_task or self._task:
            ...
            if state == self.Working:  # working
                if env.agent_idx == 0:
                    runner.buffer.rewards[runner.buffer.step - 1] = 0
                else:
                    runner1.buffer.rewards[runner1.buffer.step - 1] = 0
                # if env.agent_idx == 0:
                    # if env.hold:
                    #     print(env.hold.full_name)
                    # print(self._task[0])
                    # print(env.agent_idx, "working")
                return self.Working, move, msg
            elif state == MidTask.Failed:  # reassign task
                self._try_times += 1
                if self._try_times >= self.MAX_TRY_TIMES:
                    if env.agent_idx == 0:
                        runner.buffer.rewards[runner.buffer.step - 1] = -10
                    else:
                        runner1.buffer.rewards[runner1.buffer.step - 1] = -10
                    return self.Failed, move, msg
                self._task = []
            else:
                # print(runner.buffer.step-1)
                # print(runner.buffer.actions[runner.buffer.step - 1])
                # if env.agent_idx == 0:
                    # if env.hold:
                    #     print(env.hold.full_name)
                    # print(self._task[0])
                    # print(env.agent_idx, "succeed")
                # print(env.agent_idx)
                if env.agent_idx == 0:
                    runner.buffer.rewards[runner.buffer.step - 1] = 10
                    # print(runner.buffer.actions[runner.buffer.step-1],runner.buffer.obs[runner.buffer.step-1][0][0][1])
                else:
                    runner1.buffer.rewards[runner1.buffer.step - 1] = 10
                self._task.pop(0)
            if not self._task:
