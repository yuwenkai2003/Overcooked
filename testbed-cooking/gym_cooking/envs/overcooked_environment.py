...
class OvercookedEnvironment(gym.Env):
    ...
    def __init__(self, arglist):
        ...
        for k in range(1):
            total_action_space = []
            total_action_space.append(spaces.Discrete(5))
        ...
        self.share_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(share_obs_dim,), dtype=np.float32) for _ in range(1)]
        # print(self.observation_space[0], self.share_observation_space, self.action_space[0])
        self.policy = Policy(self.all_args, self.observation_space[0], self.share_observation_space[0], self.action_space[0], device='cpu')
        self.trainer = TrainAlgo(self.all_args, self.policy, device='cpu')
        self.buffer = SharedReplayBuffer(self.all_args, 1, self.observation_space[0], self.share_observation_space[0],
                                         self.action_space[0])
    def get_repr(self):
